...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      setOrders(allOrders.slice(0, 10)); // Show last 10 orders
  ...
  const calculateMetrics = (ordersList) => {
    ...
    const totalValue = ordersList.reduce((sum, order) => sum + (order.totalOrderValue || 0), 0);
    const totalQuantity = ordersList.reduce((sum, order) => sum + (order.quantity || 0), 0);

    const onTimeCount = ordersList.filter((order) => {
